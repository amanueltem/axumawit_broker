...
const filterSlice = createSlice({
  ...
  const { products, size } = action.payload; // Use size instead of sort
  // Filter products to exclude those with size 0 and meet the size condition
  const tempProducts = products.filter((product) => product.size > 0 && product.size <= size); 
  state.filteredProducts = tempProducts;
},
...
export const {
  ...
  SORT_PRODUCTS,
  FILTER_BY_CATEGORY,
  FILTER_BY_SIZE, // Added FILTER_BY_SIZE to exports
  FILTER_BY_PRICE,
} = filterSlice.actions;
